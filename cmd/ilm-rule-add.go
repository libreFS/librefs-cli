...
EXAMPLES:
  1. Add a lifecycle rule with a transition and a noncurrent version transition action for objects with prefix doc/ whose size is greater than 1MiB in mybucket.
     ...
     {{.Prompt}} lc ilm tier add minio myserver LIBREFSTIER-1 --endpoint https://warm-librefs-1.com \
         --access-key ACCESSKEY --secret-key SECRETKEY --bucket bucket1 --prefix prefix1

     {{.Prompt}} {{.HelpName}} --prefix "doc/" --size-gt 1MiB --transition-days "90" --transition-tier "LIBREFSTIER-1" \
         --noncurrent-transition-days "45" --noncurrent-transition-tier "LIBREFSTIER-1" \
         myserver/mybucket/
...
var ilmAddFlags = []cli.Flag{
	...
	cli.StringFlag{
		Name:   "storage-class",
		Usage:  "storage class for current version to transition into. libreFS supports tiers configured via `lc-admin-tier-add`.",
		Hidden: true,
	},
	cli.StringFlag{
		Name:   "tier",
