...
EXAMPLES:
  ...
  09. Copy a folder with encrypted objects recursively from Amazon S3 to libreFS cloud storage using s3 encryption.
      {{.Prompt}} {{.HelpName}} --recursive --enc-s3 "s3/documents" --enc-s3 "myserver/documents" s3/documents/ myserver/documents/

  10. Copy a folder with encrypted objects recursively from Amazon S3 to libreFS cloud storage.
      {{.Prompt}} {{.HelpName}} --recursive --enc-c "s3/documents/=MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTIzNDU2Nzg5MDA" --enc-c "myserver/documents/=MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTIzNDU2Nzg5BBB" s3/documents/ myserver/documents/
